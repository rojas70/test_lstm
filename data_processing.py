...
def split_data(data, train_pos=0.7, val_pos=0.8, test_pos=1.0):
    ...
    # test_data = data[-2:-1] #todo: test only one trajectory
...
def prepare_test_seqs_data(seqs, time_steps, out_time_steps):
    """
    Given the number of `time_steps` and some data,
    prepares training, validation and test data for an lstm cell.
    """
...
def generate_data(file_name, time_steps, out_time_steps):
    ...
    print('\ntest_seqs info:')
    test_x, test_y = prepare_test_seqs_data(test_seqs, time_steps, out_time_steps)
